...
│   ├── architecture/     ← ★ 产品架构与工程架构设计（产品验收/工程实现边界）
...
每个迭代周期严格按以下 5 步执行，**每步都需要深入思考、产出规划文档、工作完成后汇报，经总控指导后才能进入下一步**：
...
1. **开始任何实现前，必须先读取 `docs/architecture/graph-wiki 架构设计.md` 的产品目标与验收标准**，确保实现服务产品主线
2. **模块、代码、流水线和数据结构设计以 `docs/architecture/graph-wiki 工程架构设计.md` 与 `docs/design/` 为直接依据**，由 AI/OMX 负责设计和审核
3. 用户验收只看产品设计标准是否达成；不要把模块实现细节作为用户验收前置条件
4. 代码质量是硬性要求
