...
║  graphify.build()      ← 构建 NetworkX Graph                    ║
║       │                                                          ║
...
5. **聚类以类/文件为单位，输出含方法级业务点** — 域划分在类级进行（~2,000 节点，避免方法级噪音淹没聚类），但每个域内提取 Controller/Service 的 public 方法作为业务点（~500-800 个），方法级调用链保留在 graph.json 中按需下钻
...
| 文件 | 格式 | 生成方式 | 用途 |
|------|------|---------|------|
| `graph.json` | NetworkX node-link JSON | 自动（复用 graphify） | Agent 代码查询 |
...
└───────────┼───────────────────────────────────────────────┘
            │
            │  graph.json (34K 节点, 88K 边)  ← build() 的直接输出
