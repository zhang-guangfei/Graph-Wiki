...
> 日期：2026-06-30
> 状态：工程设计 / AI 负责设计与审核
> 来源：从 [`graph-wiki 架构设计.md`](./graph-wiki%20架构设计.md) 拆分出的模块、代码、数据流、产物结构和工程质量内容。
>
> **审核边界**：本文档不作为用户验收产品体验的主文档。产品验收以 [`graph-wiki 架构设计.md`](./graph-wiki%20架构设计.md) 中的产品架构和产品验收标准为准。本文档由 AI/OMX 团队负责设计、维护、审查和实现闭环。
...
1. [工程文档定位](#一工程文档定位)
2. [总体架构与图数据角色](#二总体架构与图数据角色)
3. [数据流与构建流程](#三数据流与构建流程)
4. [数据结构与输出规范](#四数据结构与输出规范)
5. [模块设计](#五模块设计)
6. [工程消费链路](#六工程消费链路)
7. [工程质量保障](#七工程质量保障)
8. [性能与成本评估](#八性能与成本评估)
9. [实施路线图](#九实施路线图)
10. [工程设计决策记录](#十工程设计决策记录)
11. [与 Graphify 的工程关系和差距](#十一与-graphify-的工程关系和差距)
12. [项目结构](#十二项目结构)
13. [关联文档](#十三关联文档)
...
本文档承接所有不属于产品方审核范围的内容，包括：

- 模块职责、模块边界和模块依赖。
- 代码层数据结构、JSON 产物细节、字段定义和流水线步骤。
- 复用 Graphify 的具体方式。
- API/字段/聚类/标注/导出/可视化/流水线等实现方案。
- 工程质量、性能、成本、项目结构和实现决策。

产品方只需审核产品架构文档是否达到产品标准；工程实现是否满足该标准，由 AI/OMX 团队基于本文档负责。

## 二、总体架构与图数据角色

本章合并原“核心架构”、“graph.json 角色定位”和“三层粒度模型”。它们共同定义 Graph-Wiki 的技术骨架：底层复用 graphify AST/图能力，中层构建业务域和证据，产品层输出 Domain Read Model、Workbench 和 Wiki。
...
本章原名“输出规范”。v1 起，`domain-read-model.json` 是产品体验优先级最高的数据契约，旧 `domains.json`、`field-map.json`、`manifest.json` 等继续作为分析产物和兼容输出。

### 6.1 domain-read-model.json 规范

`domain-read-model.json` 承载 Workbench v1 的业务域深度阅读模型，核心结构包括：`domains`、`flows`、`businessRules`、`fieldRules`、`evidenceIndex`、`quality`。详细字段契约见 [`domain-read-model-contract.md`](../design/domain-read-model-contract.md)。发布门禁必须校验 `build-report.json.productQuality` 与 `domain-read-model.json.quality` 一致。
...
本章从实现职责角度描述各模块。顺序已调整为服务 v1 主链路：复用/聚类 → 证据和 Domain Read Model → 字段/API/标注 → 输出和流水线。

<a id="41-复用层-reuse-layer"></a>
### 7.1 复用层（Reuse Layer）
...
Evidence 模块负责把 API、代码位置、字段、文件、配置等来源归一为稳定证据引用，供 flow step、business rule、field rule 通过 `evidenceRefs` 追溯。它是 Domain Read Model 防幻觉和可审查性的基础。

### 7.4 Domain Read Model Builder

Domain Read Model Builder 负责把聚类、API 映射、字段链路、LLM 标注和证据索引合成为 `domain-read-model.json`，并计算 `quality.deepReadingStatus`、规则风险和证据完整度。

<a id="47-字段映射模块-fieldmapper"></a>
### 7.5 字段映射模块（FieldMapper）
...
<a id="46-api-映射模块-apimapper"></a>
### 7.6 API 映射模块（ApiMapper）
...
<a id="44-wiki-导出模块-export"></a>
### 7.8 Wiki 导出模块（Export）
...
本章描述人类、Agent 和 CI 如何消费 Graph-Wiki 输出。v1 产品体验以 Workbench 的业务域深度阅读页为主，Wiki 和 Agent 查询作为补充入口。
...
本章将质量保障提前为产品架构的一部分。Graph-Wiki 的发布判断不能只看构建是否成功，还必须看 Domain Read Model 是否可深度阅读。
...
### 9.2 Release Quality Gate

统一发布入口为 `scripts/release_quality_gate.py`，CI 入口为 `.github/workflows/ci.yml`。门禁覆盖 pytest、fullstack-enterprise 构建、svn-platform smoke、Workbench build、前端构建和关键产物校验。

### 9.3 build.status 与 productQuality 的区别

`build.status` 表示流水线执行成功；`productQuality.deepReadingStatus` 表示产品阅读质量。release gate 必须拒绝缺失或与 `domain-read-model.json.quality` 不一致的 `productQuality`。

### 9.4 fullstack-enterprise 验收

fullstack-enterprise 是 v1 的主要通过样本，必须达到 `productQuality.deepReadingStatus=passed`。

### 9.5 svn-platform smoke 验收

svn-platform 作为真实复杂项目 smoke 样本，允许产品质量为 warning，但必须构建成功，并显式暴露 warning 原因。

### 9.6 生成物污染治理

`node_modules/`、`dist/`、本地 smoke 输出和日志不得进入版本控制。该规则由 `.gitignore` 和 release gate 测试共同保障。
...
原“实施计划”描述早期模块交付阶段；产品重启后，路线图应以 Domain Read Model、规则准确率、Workbench 产品化、多语言适配和持续治理为主线。以下保留原计划作为历史基线。
...
## 十一、与 Graphify 的工程关系和差距

Graphify 是 Graph-Wiki 复用底层 AST/图能力的来源，但不是 Graph-Wiki 的产品目标。Graph-Wiki 的差异化应落在业务域深度阅读、证据链和人类可消费的产品体验上。
