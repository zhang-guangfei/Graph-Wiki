...
║                                                                  ║
║  Step 1   环境检测                                                ║
║  Step 2   文件检测    ──→  graphifyy.detect()    ← Graph-Wiki 复用 ║
║  Step 3   实体提取    ──→  graphifyy.extract()   ← Graph-Wiki 复用 ║
║  Step 4   图构建      ──→  graphifyy.build()     ← Graph-Wiki 复用 ║
║  ═══════════════════════════════════════════════════════════════  ║
...
╠══════════════════════════════════════════════════════════════════╣
║                                                                  ║
║  复用层（graphifyy 作为 Python 库调用，不运行 Graphify CLI）        ║
║  ─────────────────────────────────────────────                   ║
║  graphifyy.detect()     ← 文件检测，原封不动复用                    ║
║  graphifyy.extract()    ← AST 提取，tree-sitter 30+ 语言          ║
║  graphifyy.build()      ← 构建 NetworkX Graph                    ║
║       │                                                          ║
║       │  graph.json (中间产物，不是终点)                            ║
║       │                                                          ║
║  ═══════════════════════════════════════════════════════════════  ║
║                                                                  ║
║  新建层（Graph-Wiki 核心）                                         ║
║  ─────────────────────────                                        ║
║  graph_wiki.cluster()       ← 替换 Louvain：包前缀 + import 聚合    ║
║  graph_wiki.api_mapper()    ← 新增：前后端接口字段对接              ║
║  graph_wiki.label()         ← 替换关键词拼凑：LLM 语义标注          ║
║  graph_wiki.export()        ← 替换导出：业务 Wiki + 域级 HTML       ║
║  graph_wiki.visualize()     ← 替换 D3 毛球：域级交互可视化          ║
║  graph_wiki.docs()          ← 新增：API 文档/ER 图/数据流          ║
║                                                                  ║
╚══════════════════════════════════════════════════════════════════╝
```
...
Graph-Wiki: graphifyy 只是一个底层库，被 import 使用
            用户运行 graph-wiki build → 调用 graphifyy 的前 3 步
            → 然后用自己的后 6 步 → 产出 Wiki 文档树
```

### 1.5 设计原则

1. **LLM Wiki 是入口，graph.json 是后端** — 人类和 Agent 都通过 Wiki 进入，需要精确代码细节时才查 graph.json
...
5. **类/文件为最小粒度** — 不产生方法级节点，默认类级视图（~2,000 节点），可按需下钻
...
│             · llm_label() — 为每个域生成名称和描述               │
│             · 计算域间依赖强度                                  │
│             · 输出：domains.json + code-map.json               │
│                                                              │
├──────────────────────────────────────────────────────────────┤
│                                                              │
│  第 1 层    graph.json（代码思维）                               │
│  ────────   复用 graphifyy 的 detect/build/extract              │
│             · AST 提取的精确代码结构                             │
│             · 类/方法/字段级的调用关系（原始数据）                  │
│             · 不做业务解释                                      │
...
**职责**：文件检测、AST 提取、图构建。完全复用 graphifyy 的现有能力。
...
**核心函数**：`business_cluster(G, packages, language) -> dict[str, list[str]]`
...
操作: 仅保留 file_type='code' 且 label 以 '.java' / '.vue' / '.js' 结尾的节点
输出: ~2,800 个业务节点（类文件级）
...
操作:
  1. 解析每个文件的 Java 包路径
     例: com.smc.smccloud.bin.service.impl.BindataServiceImpl
         → 提取公司域名后的层级: ['smccloud', 'bin', 'service', 'impl']
  
  2. 取 smccloud/ops 之后的第 1~2 级作为候选域键
     例: 'smccloud.bin' → 候选域 'bin'
         'smccloud.order' → 候选域 'order'
         'ops.delivery' → 候选域 'delivery'
  
  3. 合并相似候选域
     - 'bin' + 'bindata' + 'binorder' → 'Bin数据管理'
     - 'order' + 'preorder' + 'bigorder' → '订单管理'
     - 相似度判断: 关键词重叠 + 包路径前缀重叠

Step 3 — import 聚合调整
────────────────────────
输入: import 关系 (从图的 edges 中提取 relation='imports' 的边)
操作:
  1. 统计跨候选域的 import 次数
  2. 如果两个候选域的互相 import > 阈值 (如 > 30% 的类有跨域 import) 
     → 合并为同一业务域
  3. 如果候选域内部 import 很稀疏 (内聚度 < 阈值) 
     → 拆分为子域
```

**关键设计决策**：

- **不使用方法级边**：仅使用 `imports` 和 `calls` 中两端都是 `file_type='code'` 且 `label` 以 `.java`/`.vue` 结尾的边
- **噪音在 Step 1 就过滤**：方法调用、JDK 类型、Config/Util 等永远不进入聚类流程
- **合并阈值可配置**：`merge_threshold` 默认 0.3，可根据项目类型调整
...
```python
def business_cluster(G, packages, language="java"):
    # Step 1: 过滤噪音
    business_nodes = [
        n for n in G.nodes(data=True)
        if is_business_file(n)  # label 以 .java/.vue/.js 结尾
        and not is_noise(n)     # 排除 Config/Util/Constants
    ]
    
    # Step 2: 包前缀聚类
    candidates = defaultdict(list)
    for node in business_nodes:
        pkg = extract_package_path(node, language)
        # Java:  'com.smc.smccloud.bin.service.impl'
        # JS:    'src/api/binorder.js'
        # Python: 'ops.bin.service'
        domain_key = extract_domain_key(pkg, language)
        # Java:  'bin'  (公司域名后第 1 级)
        # JS:    'binorder'  (api 目录下的文件名)
        # Python: 'bin'  (项目根后第 1 级)
        candidates[domain_key].append(node)
    
    # Step 3: import 聚合调整
    domains = merge_similar_candidates(candidates, G)
    domains = split_loose_domains(domains, G)
...
**核心函数**：`llm_label(domains, G, backend="claude") -> dict`

**输入**：
- `domains`：聚类模块的输出
- `G`：NetworkX 图（用于获取节点详情）
- `backend`：LLM 后端（"claude" | "openai" | "gemini"）
...
**核心函数**：`export_wiki(domains, labels, G, output_dir) -> Path`
...
**核心函数**：`export_domain_html(domains, labels, G, output_path) -> Path`
...
```python
def match_frontend_backend(frontend_apis: dict, backend_controllers: dict) -> list:
    """
...
```python
def build_field_map(api_map: dict, entity_map: dict) -> dict:
    """
    串联 api_mapper 和 field_mapper 的输出
    生成每个前端字段的完整六层链路
    """
```
          ...
          "api_mappings": [
            {
              ...
              "controller_param": "BindataApplyDTO.supplierCode",
              "frontend_caller": "csstock/apply/index.vue",
              "frontend_field": "supplierCode"
            },
            {
              "api_url": "POST /api/binorder/detail",
              "controller_param": "BinOrderQueryDTO.supplierCode",
              "frontend_caller": "csstock/query/index.vue",
              "frontend_field": "supplierCode"
            }
          ],
          "value_rules": [
            {"source": "Excel 列 31", "priority": 1, "condition": "batch 导入模式"},
            {"source": "mainSupplierCode", "priority": 2, "condition": "默认"}
          ]
        }
      ]
    }
...
def build(root_path, name=None, language="auto", 
    ...
    1. detect_corpus()     ← 复用 graphifyy
    2. extract_ast()       ← 复用 graphifyy  
    3. build_graph()       ← 复用 graphifyy
    4. business_cluster()  ← Graph-Wiki 核心
    5. api_mapper()        ← 前后端接口对接
    6. llm_label()         ← Graph-Wiki 核心 (if llm=True)
    7. export_wiki()       ← Graph-Wiki 核心
    8. export_domain_html()← Graph-Wiki 核心
    """
```
...
└──────────┬───────────┘
           ...
           ▼
┌──────────────────────┐
│ 3. build_graph()     │  复用 graphify.build
│    → NetworkX Graph  │
│    → graph.json      │
└──────────┬───────────┘
           │
...
│    拆分 import 稀疏的候选域            │
│                                      │
│  → domains: {name → [node_ids]}     │
└──────────┬───────────────────────────┘
           │
           ▼
┌──────────────────────────────────────┐
│ 5. llm_label()                      │  ★ Graph-Wiki 核心
│                                      │
...
└──────────┬───────────────────────────┘
           ...
           ▼
┌──────────────────────────────────────┐
│ 7. export_domain_html()             │  ★ Graph-Wiki 核心
│                                      │
...
代码变更
    │
    ▼
detect_incremental()  ← 对比 manifest，找出变更文件
    │
    ├── 无变更 → 跳过
    │
    ├── 仅代码文件变更
...
| 文件 | 格式 | 生成方式 | 用途 |
|------|------|---------|------|
| `graph.json` | NetworkX node-link JSON | 自动（复用 graphifyy） | Agent 代码查询 |
| `domains.json` | 自定义 JSON | 自动（clustering + LLM） | 业务域数据 |
| `manifest.json` | 文件哈希表 | 自动 | 增量更新 |
...
- **文件拆分是关键**——`api-map.json` 全量 20K tokens 会毁掉简单查询的成本优势
...
║     ▼             ▼             ▼              ▼                 ║
║  ┌────────┐ ┌───────────┐ ┌──────────┐ ┌────────────┐           ║
║  │cluster │ │  label    │ │  export  │ │ visualize  │           ║
║  │ (过滤  │ │  (LLM     │ │  (Wiki   │ │  (域级     │           ║
║  │  +包前  │ │   标注)   │ │   导出)  │ │   HTML)    │           ║
║  │  缀聚类)│ │           │ │          │ │            │           ║
║  └───┬────┘ └─────┬─────┘ └────┬─────┘ └─────┬──────┘           ║
...
| 聚类 | Louvain（graph.json 之上） | 包前缀 + import（graph.json 之上） |
| 标注 | 关键词拼凑 | LLM 语义生成 |
...
│           ▼                                               │
│  ┌─────────────────┐                                      │
│  │ build()         │  构建 NetworkX Graph                 │
│  │                 │  输入: extraction {nodes, edges}      │
│  │                 │  输出: nx.Graph (34K 节点, 88K 边)    │
│  └────────┬────────┘                                      │
│           │                                               │
│           ▼                                               │
│  ┌─────────────────┐                                      │
│  │ to_json()       │  导出为标准 node-link JSON            │
│  │                 │  输出: graph.json                     │
│  └────────┬────────┘                                      │
│           │                                               │
└───────────┼───────────────────────────────────────────────┘
            │
            │  graph.json (34K 节点, 88K 边)
            │  这是 Graphify 的终点，Graph-Wiki 的起点
            │
┌───────────┴───────────────────────────────────────────────┐
│                                                           │
...
┌───────────┴───────────────────────────────────────────────┐
│                                                           │
│  第 3 层：导出层（graph_wiki.export + visualize）            │
│  ─────────────────────────────────────────                │
│                                                           │
│  ┌──────────────────────┐  ┌──────────────────────┐       │
...
│  │ docs.py (可选)        │                                 │
│  │                      │                                 │
│  │ · api-docs.md        │  从 Controller 注解提取 API     │
│  │ · data-flow.md       │  从 field-map.json 生成        │
...
│  │ quality.py (可选)     │                                 │
│  │                      │                                 │
│  │ · quality-report.md  │  大方法/大实体/循环依赖          │
│  │                      │                                 │
│  │ Token: 0 (静态分析)   │                                 │
│  │ LLM 增强: 按需触发     │                                 │
│  └──────────────────────┘                                 │
...
### 8.5 各层使用的 graph.json 子集

```
graph.json (全量, 34K 节点, 88K 边)
    │
    ├── 聚类层使用:
    │    类节点 (~2,800) + imports 边 (11,335)
    │    → 域划分
    │
    ├── 业务点层使用:
    │    方法节点 (过滤后 ~5,000-8,000) + calls 边 (14,542) + contains 边 (4,645)
    │    → 业务点提取 + 跨域调用分析
    │
    ├── 字段映射层使用:
...
首次构建 graph-wiki build ops-backend:
  ...
  llm_label()        ▏ ~50K in   ~30s     LLM API (★唯一消耗 Token 的步骤)
                     ▏ ~7.5K out
  export_wiki()      ▏ 0 Token   ~5s      模板生成
  export_html()      ▏ 0 Token   ~3s      D3 模板
  ──────────────────────────────────────
  合计               ~57,500 Token   ~12-15min  成本 ~$0.3-1 (一次性)
```
...
**第 3 层 — wiki/采购管理/业务点/采购申请提交.md（业务点详情）**：
```markdown
# 采购申请提交
...
| 过滤规则可能遗漏边缘情况 | 🟢 低 | 提供手动调整 `biz-filter.yaml` 配置 |
...
|------|---------------|-----------------|:--:|------|
| **多语言 AST 提取** | tree-sitter 支持 30+ 语言 | 优先实现 Java + Vue，其他语言逐步支持 | 🟡 中 | 复用 graphifyy 的多语言能力，只是聚类模块先不做 |
...
### 决策 1：为何复用 graphifyy 而不是从零写 AST 提取

**选择**：复用 graphifyy 的 `detect()` / `extract()` / `build()`

**理由**：
- graphifyy 的 tree-sitter 解析器支持 30+ 种语言，经过大量项目验证
- 自研 AST 提取的成本远高于复用，且无差异化价值
- Graph-Wiki 的核心创新在聚类和标注，不在 AST 解析
- graphifyy 是 MIT 许可的 Python 包，可以自由使用

**权衡**：依赖一个外部包有版本兼容风险。缓解措施：锁定 graphifyy 主版本号。
...
**权衡**：包结构不规范的项目效果可能差。缓解措施：提供 `--domain-map` 手动映射配置。

### 决策 3：为何类/文件为最小粒度

**选择**：聚类和 Wiki 的最小单位是类/文件，不是方法
...
- [Graphify 缺陷深度分析](../reference/graphify-limitations-analysis.md) — 为什么要做 Graph-Wiki 的量化依据
- [LLM Wiki 设计原则](./llm-wiki-design-principles.md) — Part B > Part A 的方法论
- [三层文档体系方案](./three-layer-document-system.md) — Backstage 应用层的设计
- [市面工具对比](../reference/code-analysis-tools-comparison.md) — 竞品分析和市场定位
- [Graph-Wiki 路线图](./graph-wiki-roadmap.md) — 方案 A vs B 的决策过程
