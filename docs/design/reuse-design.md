...
Graph-Wiki 定位为 graphify 的**增量替代**而非完全重写。graphify 在文件检测（30+ 语言、tree-sitter AST 解析、.graphifyignore 支持）和图构建（NetworkX 序列化、节点去重、增量合并）方面已经具备成熟且经过生产验证的能力。直接复用这些能力可以：
...
│  graphify.build.build_from_json()        NetworkX 图构建          │
│  graphify.build.build_merge()            增量图合并                │
...
def build_graph(
    ...
    *,
    ...
    """构建 NetworkX 图：将 AST 提取结果序列化为可查询的图结构。

    这是 graphify 流水线中 build 步骤的直接封装。
    生成的图保存为 graph.json（node-link 格式），作为后续所有模块的数据源。
...
## 4. graph.json 节点/边结构详解

### 4.1 节点结构

每个节点是一个 dict，在 graph.json 的 `"nodes"` 列表中存储：
