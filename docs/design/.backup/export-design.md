...
for domain in domains:
    ...
    # name 其次（LLM 标注的英文 ID，如 "purchase"）
...
def _write_index(domains: list[Domain], output_dir: Path):
    lines = [
        "---",
        f'project: "{_detect_project_name(domains)}"',
    ...
    for d in sorted(domains, key=lambda x: x.anchors_count(), reverse=True):
        ...
        deps = ", ".join(
            f"[[{_resolve_dep_name(dep_id, domains)}]]"
            for dep_id in list(d.dependencies.keys())[:3]
        ) or "—"
...
def _write_data_flow(domain: Domain, field_map: dict, domain_dir: Path):
    # 依次尝试 domain.id, domain.name, domain.display_name
    key = domain.id or domain.name or domain.display_name
    tables = field_map.get(key, {})
    if not tables:
        # 域名为中文时也尝试查找
...
def _write_dependencies(domain: Domain, domain_dir: Path):
    ...
    # 计算各强度级别的数量
    strong = sum(1 for c in deps.values() if c >= 50)
    medium = sum(1 for c in deps.values() if 20 <= c < 50)
    weak = sum(1 for c in deps.values() if c < 20)
    ...
    # 按 import 次数降序排列
    for dep_id, count in sorted(deps.items(), key=lambda x: -x[1]):
        strength = _dependency_strength(count)
        dep_display = f"[[{_resolve_dep_name(dep_id, domains)}]]"
...
def test_export_wiki_basic(tmp_path):
    ...
    domains = [
        Domain(
            ...
            dependencies={"inventory": 45, "supplier": 12},
            total_files=30,
        ),
        Domain(
            id="inventory",
