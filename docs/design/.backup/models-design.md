...
class Domain:
    ...
    name: str = ""                               # LLM 标注的中文名（如 "Bin数据管理"），空字符串表示未标注
    ...
    dependencies: dict[str, int] = field(default_factory=dict)  # 对其他域的 import 依赖 {domain_id: import_count}
...
| `name` | `str` | `""` | label | `label_domains()` | LLM 生成的中文域名。空字符串 = 未标注 |
...
| `dependencies` | `dict[str, int]` | `{}` | cluster | `adjust_by_imports()` | import 依赖统计。key 为目标域 ID，value 为 import 次数 |
...
| `label.py` | `Domain`, `BusinessPoint` | 修改 `Domain.name`, `BusinessPoint.display_name` |
...
def test_domain_json_serialization():
    ...
    assert parsed["dependencies"]["inventory"] == 42
```
