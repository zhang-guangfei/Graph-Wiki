...
| `Domain.dependencies` | 连线数据：[{domain, name, import_count, strength}] |
...
| **部分域无依赖** | `d.dependencies` 为空 list | 该域作为孤立节点显示（无连线），仍渲染为圆形节点 |
...
def test_export_domain_html_normal():
    ...
    domains = [
        ...
        Domain(
            ...
            total_files=15,
            dependencies=[{"domain": "bin-data", "import_count": 10}],
        ),
        Domain(
            id="supplier",
            name="supplier",
...
def test_export_domain_html_skips_missing_target():
    ...
    domain_a = Domain(
        id="domain-a",
        dependencies=[{"domain": "domain-b", "import_count": 10}, {"domain": "nonexistent", "import_count": 5}],  # domain-b 不在列表中
        anchors={"controller": [{"_role": "controller"}]},
    )
