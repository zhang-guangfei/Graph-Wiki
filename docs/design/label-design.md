...
- `wiki/{domain.display_name or domain.name or domain.id}/summary.md` 文件已生成
...
def _build_prompt(domain: Domain, backend_root: Path, config: LabelConfig) -> str:
    ...
    deps = [
        f"- {dep.get('domain', '?')}: {dep.get('import_count', 0)} 次 import"
        for dep in sorted(domain.dependencies, key=lambda x: -x.get("import_count", 0))[:5]
    ]
...
```python
def _write_summary(domain: Domain, result: dict, backend_root: Path):
    """根据 LLM 标注结果生成 summary.md（委托 safe_write_summary 写入）"""
    summary_path = Path("wiki") / (domain.display_name or domain.name or domain.id) / "summary.md"
    ...
    # 委托 safe_write_summary 写入（含锁定保护）
    full_content = "\n".join(lines)
    safe_write_summary(summary_path, full_content)
```
...
def test_build_prompt_contains_all_sections():
    ...
    domain = Domain(
        ...
        dependencies=[{"domain": "inventory", "import_count": 10}, {"domain": "supplier", "import_count": 5}],
    )
