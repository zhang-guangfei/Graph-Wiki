...
- CI 入口统一为 `.github/workflows/ci.yml`，PR/push 可复用同一脚本并上传 `output/release-quality-gate/` 验收证据。
...
python3 -m graph_wiki.pipeline build tests/fixtures/fullstack-enterprise --no-llm --output-dir output/release-quality-gate/fullstack-enterprise
python3 -m graph_wiki.pipeline build tests/svn-platform --no-llm --output-dir output/release-quality-gate/svn-platform
```
