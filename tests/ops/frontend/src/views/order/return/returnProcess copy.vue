<template>
  <div class="maindiv">
    <el-row class="btu">
      ...
      <!-- <el-button size="mini" plain style="margin-left: 2%" icon="el-icon-edit-outline" @click="dialogFormVisible = true">修改手续费</el-button>
      <el-button size="mini" plain style="margin-left: 2%" icon="el-icon-edit" @click="executeBack()">执行退货</el-button> -->
    </el-row>
    ...
    <div class="tab">
      <el-table
        ...
        <el-table-column
          :index="(page.pageNumber-1)*page.pageSize+1"
          fixed
          type="index"
          width="50" />
        ...
        <el-table-column fixed label="申请号" width="110" show-overflow-tooltip>
          <template slot-scope="scope">
            <el-link type="primary" @click="handdle(scope.row)">{{ scope.row.applyNo }}</el-link>
          </template>
        </el-table-column>
        ...
        <el-table-column
          prop="requestWarehouseCodeName"
          label="要求退货仓库"
          width="140"
          show-overflow-tooltip/>

        <el-table-column
          prop="rcvWarehouseCodeName"
          label="实际退货仓库"
          width="140"
          show-overflow-tooltip/>
        ...
        <el-table-column
          :formatter="dateFormatterForInvoiceDate"
          prop="invoiceDate"
          label="开票日期"
          width="120"
          show-overflow-tooltip/>
        ...
        <el-table-column
          :formatter="dateFormatterForApplyTime"
          prop="applyTime"
          label="申请时间"
          width="120"
          show-overflow-tooltip/>
        ...
        <el-table-column
          :formatter="toUserStockFormatter"
          prop="toUserStock"
          label="退回用户专备"
          width="97"
          show-overflow-tooltip/>
        ...
        <el-table-column
          :formatter="dateFormatterForInTime"
          prop="inTime"
          label="入库时间"
          width="120"
          show-overflow-tooltip/>
        ...
        <el-table-column
          :formatter="dateFormatterForReceiveTime"
          prop="receiveTime"
          label="收货时间"
          width="140"
          show-overflow-tooltip/>
        ...
        <!--add by LiYingChao from 20221103 bugId 8421-->
        <el-table-column
          prop="returnType"
          label="退货类别"
          width="110"
        ...
        <el-table-column
          ...
          show-overflow-tooltip>
          <template slot-scope="scope">
            <el-button v-if="existFile(scope.row.fileName)" plain type="primary" size="mini" @click="downLoadFile(scope.row.fileRealName, scope.row.applyNo)">下载附件</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
import { fetchList as queryData, backOrder, printReturnOrder, rcvOrderReGister, refuseRcv, exportReturnOrder, cancelReturnOrder, creatApplyNo, addReturnOrder, getWareHouseByCustomerNo, downloadFile } from '@/api/order/returnorder'
...
export default {
  ...
  methods: {
    ...
    // 获取收货登记界面的权限
    getReceivingAuth() {
      // getDictByClassCodeAndCode('9002', '5').then(res => {
      //   const roles = res.content.extNote1
      //   this.receivingAuth = roles.split(',')
      //   console.log('收货登记功能所需权限=>', this.receivingAuth)
      //   this.showReceiving = this.receivingAuth.some(r => this.roles.includes(r))
      //   console.log('auth => ', this.showReceiving)
