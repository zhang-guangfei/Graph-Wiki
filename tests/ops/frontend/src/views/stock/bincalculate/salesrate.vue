  ...
  <div class="app-container">
    <div>
      <el-form ref="form" :inline="true" :model="form" size="small" label-width="100px">
        <el-form-item>
          <el-select v-model="form.stockType" placeholder="请选择" style="width:125px" size="mini">
            <el-option
        ...
        <el-form-item>
          <el-input v-model="form.stockcode" size="mini" style="width:190px" placeholder="All/仓库代码/客户代码/代理代码" />
        </el-form-item>
        <el-form-item>
          <el-input v-model="form.modelNo" size="mini" style="width:200px" placeholder="型号" />
        </el-form-item>
        ...
        <el-form-item>
          <el-button v-permission="['732139']" type="primary" icon="el-icon-search" size="mini" @click="initData">查询</el-button>
          <!-- <el-button icon="el-icon-search" title="搜索" size="mini" @click="initData" /> -->
        </el-form-item>

        <el-form-item>
          <span> {{ '计算完成时间:' }} </span>
          {{ mainData.finishTime }}
          统计范围:{{ mainData.dateFromDate }}至{{ mainData.dateToDate }}
        </el-form-item>

        <el-form-item style="float:right">
          <el-button v-permission="['362393']" v-loading="exportLoading" type="primary" icon="el-icon-upload2" size="mini" @click="exportModelExpFreq">导出</el-button>
        </el-form-item>
      </el-form>
      ...
      >
        ...
        <el-table-column fixed label="型号" width="150" show-overflow-tooltip>
          <template slot-scope="scope">
            <!-- <el-popover placement="right" width="600">
              <el-table v-loading="listDetailLoading" :data="detaildata" :row-style="{height: '0'}" border mini style="width: 100%;font-size:10px">
              ...
              >{{ scope.row.modelNo }}</span>
            </el-popover> -->
            <el-link type="primary" @click="showdetail(scope.row)">{{ scope.row.modelNo }}</el-link>
          </template>
        </el-table-column>
        <el-table-column
          fixed
      ...
      <pagination v-show="form.total>0" :total="form.total" :page-sizes="[20, 50, 100, 200, 500]" :page.sync="form.pageNum" :limit.sync="form.pageSize" @pagination="initData" />
...
import { listModelExpFreq, listModeldetail, exportModelExpFreq, getModelExpFreqMain } from '@/api/stock/binorder'
...
export default {
  ...
  watch: {
    ...
    $route: {
      ...
      handler(newVal, oldVal) {
        if (newVal && newVal.name === 'BinCalculate' && this.$route.query.active === 'salesratePage') {
          this.form.modelNo = this.$route.query.modelNo
          this.form.stockcode = this.$route.query.warehouseCode
          this.form.stockType = this.$route.query.stockType // 0:All 1:物流中心 2:分库 3:客户
          this.initData()
        }
      }
    }
  ...
  methods: {
    ...
    initData() {
      ...
      //     this.$message.error('All/仓库代码/客户代码/代理代码框不能为空！')
      //     this.listLoading = false
      //     return
      //   }
      listModelExpFreq(this.form).then(result => {
        var data = result.content
        this.applyData = data.list
    ...
    exportModelExpFreq() {
      ...
      //   const stockName = codeFormatter(this.stockTypeData, this.form.stockType)
      const expdate = this.form.stockcode + this.form.modelTYpe + this.dayjs(new Date()).format('YYYY-MM')
      if (this.form.stockType > 2 && this.form.stockcode !== '') {
        this.fileName = expdate + '.csv'
      } else {
        this.fileName = expdate + '.zip'
      }
      exportModelExpFreq(this.form).then(res => {
    ...
    getModelExpFreqMain() {
      getModelExpFreqMain().then(result => {
        if (result.success && result.content != null) {
          this.mainData = result.content
        }
      }).catch(error => {
        console.log(error)
      })
