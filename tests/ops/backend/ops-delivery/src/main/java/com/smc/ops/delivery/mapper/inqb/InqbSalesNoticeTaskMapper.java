...
import com.sales.ops.dto.inqb.InqbUpTaskInfoVO;
...
@DS("sharedb")
@Mapper
public interface InqbSalesNoticeTaskMapper {


    @Select(" <script> select * from ops_sales_notice_task where batch_no = #{batchNo} </script>")
    InqbUpTaskInfoVO getSalesNoticeTask(@Param("batchNo") String batchNo);
    ...
    @Update(" <script> update ops_sales_notice_task set " +
            ...
            "  " +
            "</script> ")
    int updateSalesNoticeTask(InqbUpTaskInfoVO inqbUpTaskInfoVO);
