...
public class DateUtil {
    ...
    /**
     * 将短时间格式时间转换为字符串 yyyy-MM-dd 获取上下午
     *
     * @param dateDate
     * @return
     */
    public static String dateToDateStringAndMsg(Date dateDate, int hourCondition,String defaultMsg,String eligibleMsg) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(dateDate);
        int hour = calendar.get(Calendar.HOUR_OF_DAY);
        String msg = defaultMsg;
        if(hour == hourCondition){
            msg = eligibleMsg;
        }
        SimpleDateFormat formatter = new SimpleDateFormat(DATE);
        return formatter.format(dateDate) + msg;
    }
    ...
    /**
     * 将字符串YYYY-MM-DD格式时间转换为字符串yyMMdd
     *
     * @param dateStr 日期
     * @return yyMMdd
     * YYMMDD
     */
    public static String getYearMonthDayByString(String dateStr) {
        if (StringUtils.isBlank(dateStr)) {
            return null;
        }
        Date date = stringToDate(dateStr);
        String returnVal = getYearMonthDay(date);
        return returnVal.substring(2, returnVal.length());
    }

    public static Date geDayByYYMMdd(String dateStr) {
        if (StringUtils.isBlank(dateStr)) {
            return null;
        }
        String returnVal = getYearMonthDay(new Date());
        dateStr = returnVal.substring(0, 2).concat(dateStr);
    ...
    public static String convertToYYYYMMDD(String dateStr) {
        try {
            ...
            SimpleDateFormat outputFormat = new SimpleDateFormat(DATE);
            // 解析输入的日期字符串为 Date 对象
    ...
    // 将日期转换为时间戳，按照天来
    public Long dateToTimestamp(Date dateLong) {
        Date dateToday=  getDate(dateLong);
...
//        System.out.println(dateDiff(stringToDateTime("2024-11-04 14:09:00")));
//        System.out.println(dateDiff(stringToDateTime("2024-11-05 18:30:00")));
//        System.out.println(dateDiff(stringToDateTime("2024-11-06 14:09:00")));
        System.out.println("241210:" + convertToYYYYMMDD("241210"));
        System.out.println("241299:" +convertToYYYYMMDD("241299"));
        System.out.println("555555:"+convertToYYYYMMDD("555555"));
    }
