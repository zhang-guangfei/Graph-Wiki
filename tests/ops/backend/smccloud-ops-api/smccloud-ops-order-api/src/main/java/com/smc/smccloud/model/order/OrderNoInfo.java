...
public class OrderNoInfo {
	...
	/**
	 * bug14549放行信用拦截订单时，对老订单号进行解析
	 * 
	 * @param no
	 */
	public String releaseOldNo(String no) {
		if (no.length() == 10 && !no.contains("-")) {
			this.orderNo = no.substring(0, 7);
			this.itemNo = Integer.parseInt(no.substring(7, 10));
			...
			this.fullOrderNo = this.orderNo + "-" + this.itemNo.toString();
		} else if (no.contains("-")) {
			this.fullOrderNo = no;
		}
	...
	/**
	 * 转化日本订单号 jporderNo=AG000002 jpitemNo=010 jporderNo=AG000002-2 jpitemNo=1
	...
	public OrderNoInfo convertJPOrder(String jporderNo, String jpitemNo) {
		...
		try {
			...
			if (this.PURCHASE_TYPES.contains(jporderNo.substring(0, 1)) && !jporderNo.startsWith("AP")) {
				// 采购类型
	 ...
	 * 可以转化单号
	 * <p>
	 * poNo=G12345678-20 itemNo=1 poNo=9900000 itemNo=010 poNo=9900000010
	 * itemNo=null
	 *
	 * @param poNo
	 * @param itemNo
	 *            new OrderNoInfo().convertPOOrder("G123456-10-4","1")
	 *            OrderNoInfo(orderType=null, orderNo=G123456, itemNo=10,
	 *            splitItem=4, fullOrderNo=G123456-10-4, purchaseType=null,
	 *            strItemNo=010, orFullOrderNo=null, poNo=G123456-10-4,
	 *            poItemNo=1, isError=false)
	 * @return
	 */
	...
	/**
	 * 可以解析订单号： G123456-12-1 G123456010
	 *
	 * @param fullOrderNo
	 * @return
	 */
	...
	public static void main(String[] args) {
		OrderNoInfo orderNoInfo = new OrderNoInfo().convertFullOrderNo("11838097-6-2");
		// // OrderNoInfo orderNoInfo2 = new
		// OrderNoInfo().convertFullPONo("11270078-2-3 ");
		// String s = String.valueOf(orderNoInfo.getItemNo());
		//
		// String s1 = String.valueOf(orderNoInfo.getSplitItem());
		//
		// System.out.println("s1 = " + s1+" "+s);

		System.out.println("orderNoInfo = " + JSONObject.toJSONString(orderNoInfo));

		// System.out.println("orderNoInfo2 = " +
		// JSON.toJSONString(orderNoInfo2));
