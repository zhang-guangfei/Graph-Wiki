...
public class PreOccupiedInventoryCancelReceiver extends AdapterReceive {
    ...
    @StreamListener(value = InputChannelBinding.SMS_TO_OPS_YYQB_RECEIVER,
            condition = "headers['x-flag']=='" + Constants.ERP_PREOCCUPIEDINVENTORY_DELETE + "'")
    public void receive(@Payload Message<RabbitMqMessage> message,
        ...
        try {
            ...
            if (message != null) {
                ...
                log.info("预占库存删除-消息 {} -- {}", rabbitMqMessage.getRandomNumber(), rabbitMqMessage.getContent());
                PreSaleInventory info = JSON.parseObject(rabbitMqMessage.getContent(), PreSaleInventory.class);
                /**
                 * 调用预占库存删除接口，业务逻辑如下：释放库存
                 */

                /**
                 * 返回结果
                 */
                AdapterResult result = stockService.handlePreOccupiedInventoryCancelMQ(info);
                 ...
                 * 过滤标记：x-flag = 'erp-preOccupiedInventory-delete-return'
                 */
            ...
            if (StringUtils.isNotBlank(e.getMessage()) && (e.getMessage().startsWith("feign.RetryableException")
                ...
                log.error("预占库存删除消费失败: {}", e.getMessage());
                channel.basicNack(deliveryTag, false, true);
            } else {
                log.error("预占库存删除消费失败: {} --error: {}", message.getPayload(), e.getMessage(), e);
