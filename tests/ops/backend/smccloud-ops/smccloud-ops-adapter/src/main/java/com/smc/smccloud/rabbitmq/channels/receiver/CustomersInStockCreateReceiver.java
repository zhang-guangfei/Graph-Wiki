...
import com.smc.smccloud.model.adapter.stock.BinDataApply;
...
public class CustomersInStockCreateReceiver extends AdapterReceive {
    ...
    @StreamListener(value = InputChannelBinding.SMS_TO_OPS_YYQB_RECEIVER,
            condition = "headers['x-flag']=='" + Constants.ERP_CUSTOMERINSTOCK_CREATE + "'")
    public void receive(@Payload Message<RabbitMqMessage> message,
        ...
        try {
            ...
            if (message != null) {
                ...
                log.info("在库补库-消息 {} {}", rabbitMqMessage.getRandomNumber(), rabbitMqMessage.getContent());
                /*
                 * 调用顾客在库补库接口，如下：顾客在库补库信息写进ERP
                 */
                BinDataApply apply = JSON.parseObject(rabbitMqMessage.getContent(), BinDataApply.class);
                ...
                /*
                 * 顾客在库补库处理结果：将处理结果发送至顾客在库补库接收结果队列 返回信息如下：
                 * 顾客在库补库信息：顾客在库补库号，处理状态：success(true、false)) 过滤标记：x-flag =
                ...
                log.info("在库补库-消息消费成功: {} {} --result: {}", rabbitMqMessage.getRandomNumber(), apply.getId(), result);
            }
        } catch (Exception e) {
            if (StringUtils.isNotBlank(e.getMessage()) && (e.getMessage().startsWith("feign.RetryableException")
                ...
                log.error("在库补库消费失败: {}", e.getMessage());
                channel.basicNack(deliveryTag, false, true);
            } else {
                log.error("在库补库消费失败: {} --error: {}", message.getPayload(), e.getMessage(), e);
