...
import com.smc.smccloud.model.adapter.stock.BinTransferInfo;
import com.smc.smccloud.model.adapter.stock.BinTransferReturn;
...
public class CustomerInStockTransferCreateReceive extends AdapterReceive {
    ...
    @StreamListener(value = InputChannelBinding.SMS_TO_OPS_YYQB_RECEIVER,
            condition = "headers['x-flag']=='" + Constants.ERP_CUSTOMERINSTOCKTRANSFER_CREATE + "'")
    public void receive(@Payload Message<RabbitMqMessage> message,
        ...
        try {
            ...
            if (message != null) {
                ...
                log.info("顾客在库调库-消息 {}  {}", rabbitMqMessage.getRandomNumber(), rabbitMqMessage.getContent());

                /*
                 * 调用顾客在库调库接口，如下：顾客在库调库信息写进ERP
                 */
                BinTransferInfo info = JSON.parseObject(rabbitMqMessage.getContent(), BinTransferInfo.class);
                /*
                 * 返回结果
                 */
                List<BinTransferReturn> result = stockService.handleCustomerInStockTransferCreateMQ(info);
                ...
                log.info("顾客在库调库-消息消费成功: {} {} --result: {}", rabbitMqMessage.getRandomNumber(), info.getNo(), result);
            }
        } catch (Exception e) {
            if (StringUtils.isNotBlank(e.getMessage()) && (e.getMessage().startsWith("feign.RetryableException")
                ...
                log.error("在库调库队列,消费失败: {}", e.getMessage());
                channel.basicNack(deliveryTag, false, true);
            } else {
                log.error("在库调库队列,消费失败: {} --error: {}", message.getPayload(), e.getMessage(), e);
