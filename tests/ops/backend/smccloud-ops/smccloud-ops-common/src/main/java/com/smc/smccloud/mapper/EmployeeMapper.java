...
import com.smc.smccloud.model.EmployeeVO;
...
public interface EmployeeMapper extends BaseMapper<EmployeeDO> {
    ...
    @Select("SELECT TOP 1 O.name FROM Hr_employee_position EP " +
            "INNER JOIN HR_ORG_POSITION OP ON EP.PositionId = OP.position_id " +
            "INNER JOIN HR_organization O ON OP.ORG_ID = O.ID " +
            "WHERE EP.IsPrimaryPosition = 1 AND OP.is_deleted = 0 AND EP.EmployeeId = #{userId}")
    String getDeptNameByEmployeeId(@Param("userId") String userId);
}
