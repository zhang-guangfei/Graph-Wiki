...
public class RabbitmqRedis {
	...
	private static RedisManager redisUtil;

	public RabbitmqRedis(@Autowired RedisManager redisManager) {
		RabbitmqRedis.redisUtil = redisManager;
	}

	private static final Logger logger = LoggerFactory.getLogger(RabbitmqRedis.class);
	...
	public void consumer(RabbitMqMessage rabbitMqMessage) {
		...
		String key = keyPrefix + rabbitMqMessage.getRandomNumber();
        logger.info("[接单]redis 待消费的key {}",key);
		if(redisUtil.hasKey(key)) {
		    logger.info("[接单]redis delete key {}",key);
			redisUtil.del(key);
		}
	}
	...
	public synchronized RabbitMqMessage rabbitMqRandomNumber(String keyPrefix, RabbitMqMessage rabbitMqMessage) {
		...
		logger.info("redisUtil.hasKey(key) :" + redisUtil.hasKey(key) + ":" + rabbitMqMessage.getDataType() + ":生成随机数：" + randomNumber);
		while(redisUtil.hasKey(key)) {
			randomNumber = random(RabbitMqConstants.RABBITMQ_RANDOM);
			key = keyPrefix + randomNumber;
			logger.info("wihle redisUtil.hasKey(key) :" + redisUtil.hasKey(key) + ":" + rabbitMqMessage.getDataType() + ":生成随机数：" + randomNumber);
		}
