...
public class OrderNoUtils {
	...
	public static String hexAdd(String maxorderno, int num) {
		int len = maxorderno.length() - 1;
		// 是否需要进位
		boolean b = true;
		// 参与了运算的位，运算之后的结果字符串，
		...
		while (b) {
			...
			if (index == -1) {
				...
				if (sum < 34) {
					...
					result = maxorderno.substring(0, len) + change;
					b = false;
				} else {
