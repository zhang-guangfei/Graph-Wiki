...
def test_workbench_search_uses_domain_read_model_flows_and_apis(tmp_path: Path):
    model = {
        "schema": {"version": "domain-read-model-v1"},
        "evidenceIndex": {
            "api:POST:/orders": {"id": "api:POST:/orders", "type": "api", "label": "POST /orders", "path": "api-map.json", "confidence": 1.0, "status": "ready"},
        },
        "domains": [
            {
                "domainKey": "order",
                ...
                "summary": "处理订单。",
                "core": True,
                "flows": [{"flowId": "order.create", "title": "创建订单", "summary": "提交订单。", "steps": [], "evidenceRefs": ["api:POST:/orders"], "status": "ready", "confidence": 1}],
                "rules": [],
                "fieldRules": [],
                "evidenceRefs": [],
                "quality": {"deepReadingStatus": "passed"},
    ...
    results = ProductDataService(tmp_path).search("创建订单")

    assert any(item["type"] == "business_point" and item["domainKey"] == "order" for item in results)
